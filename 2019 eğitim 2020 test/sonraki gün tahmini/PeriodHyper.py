...
def KnnPred (x,y,cut):
    ...
    for i in range (1,lastNum+1):
        ...
        xTrain.append(x.iloc[i:364+i, :].values)    
        xTest.append(x.iloc[364+i:365+i, :].values)
        yTrain.append(y.iloc[i:364+i, :].values)
        yTest.append(y.iloc[364+i:365+i, :].values)
...
def KnnPredImportance (x,y,cut):
    ...
    for i in range (1,lastNum+1):
        ...
        xTrain.append(x.iloc[i:cut-1+i, :].values)    
        xTest.append(x.iloc[cut-1+i:cut+i, :].values)
        yTrain.append(y.iloc[i:cut-1+i, :].values)
...
yPredKnnImportance = KnnPredImportance(xForKnn,y,splint)
...
def DtrRegPredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceDtr = DtrRegPredImportance(xForDtrReg,y,splint)
...
#DTRClassifaciton
def DtrClassPredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceDtrClass = DtrClassPredImportance(xForDtrClass,y,splint)
...
def LogisticRegressionPredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceLogisticRegression = LogisticRegressionPredImportance(xForLogisticRegression,y,splint)
...
def SvmPred(x,y,cut):
    ...
    for i in range (1,lastNum+1):
        ...
        model = svm.LinearSVC(C = 0.1, multi_class = 'ovr', penalty = 'l2') #burası hatalı olabilir kontrol et
        model.fit(xTrain[i-1], yTrain[i-1])
        yPred.append(model.predict(xTest[i-1])) 
        
    return yPred

def SvmPredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceSvm = SvmPredImportance(xForSvm,y,splint)
...
def RfclassPredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceRfc = RfclassPredImportance(xForRfc,y,splint)
...
yPredImportanceNn = NnPredImportance(xForNn,y,splint)
...
def XgbHingePredImportance(x,y,cut):
    xTrain = []
    xTest = []
    yTrain = []
    yTest = []
...
yPredImportanceXgb = XgbPredImportance(xForXgb,y,splint)
yPredXgbHinge = XgbHingePred(x,y,splint)
yPredImportanceXgbHinge = XgbHingePredImportance(xForXgb,y,splint)
